...
# DEFAULT_CONFIG_DIR  = "/etc/backups"
DEFAULT_CONFIG_DIR  = "examples/mysql"
DEFAULT_CONFIG_NAME = "backups"
...
class Backups:
  ...
  def __init__(self, file):
    # dumps_dir   = os.environ.get("BACKUPS_DUMPS_DIR", self.DEFAULT_DUMPS_DIR)
    # config_dir  = os.environ.get("BACKUPS_CONFIG_DIR", self.DEFAULT_CONFIG_DIR)
    # config_name = os.environ.get("BACKUPS_CONFIG_NAME", self.DEFAULT_CONFIG_NAME)

    # if not os.path.isfile(config_dir):
    #   raise RuntimeError(f"Config dir '{config_dir}' doesn't exist.")

    # backup_file = os.path.join(config_dir, config_name + ".yaml")
    # if not os.path.isfile(backup_file):
    #   raise RuntimeError(f"Config dir '{backup_file}' doesn't exist.")
    if not os.path.isfile(file):
      raise RuntimeError(f"Backups file '{file}' doesn't exist.")

    with open(file, "r") as f:
      config = yaml.safe_load(f)
    print(config); exit(55)

    if not name in config["backups"].keys():
      raise RuntimeError(f"Backup configuration '{name}' doesn't exist.")

    self.name    = name
    self.config  = configs["backups"][name]
    self.options = self.config.get("options",    {})
    self.mysql   = Mysql(self.config["connection"])
    print(self); exit(55)


  def run(self, command):
    logging_debug(f"Running '{command}'...")
  ...
  def prepare(self, db=None):
    ...
    self.namedir = "%s-%s"    % (self.jobdir, self.timestamp)
  ...
  def backup(self, database=None):
    ...
    if db:
      logging_info(f"Backing up database {database}")
      self.dump_database(db)
    elif self.options.get("server", False) == True:
      logging_info(f"Backing up the server")
      self.dump_server()
    else:
      self.dump_databases()
...
def main():
  try:
    parser = argparse.ArgumentParser(
      ...
      description='Backups mysql based on a config',
    )
    parser.add_argument("name",
      help="The backup named config you want to use",
      type=str
    )
    parser.add_argument("-f", "--file",
      help="The backups config file to use",
      type=str,
      default=os.path.join(DEFAULT_CONFIG_DIR, DEFAULT_CONFIG_NAME + ".yaml")
    )
    ...
    backups = Backups(args.file)
    # report = backups.backup(db)
    # print(report)
